...
class StandbyLifecycleTest {
    @Test
    void keepsMicroBattlesAvailableAcrossTwelveRematchesWithALobbyPlayerOnline() {
        assertReplayChain("micro");
    }

    private static void assertReplayChain(String prefix) {
        StandbyGamePool<String> pool = new StandbyGamePool<>(StandbyRefillPolicy.TARGET_SIZE);
        assertTrue(pool.offer(prefix + "-startup"));
        ...
        for (int match = 1; match <= 12; match++) {
            ...
            assertTrue(pool.offer(prefix + "-refill-" + match));
        }
    }
}
